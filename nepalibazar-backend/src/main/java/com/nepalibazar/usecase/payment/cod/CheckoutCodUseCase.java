...
public class CheckoutCodUseCase {
    ...
    public CheckoutCodUseCaseResponse execute(String token, CheckoutCodUseCaseRequest request){
        try{
            if(token==null){
                return new CheckoutCodUseCaseResponse(-1, "Unauthorized");
            }
            ...
            Optional<UserEntity> user= userRepository.findByEmailPhone(email);
                if(user.isEmpty()){
                    return new CheckoutCodUseCaseResponse(-1, "User not available");
                }
            ...
            if(order.isEmpty()){
                return new CheckoutCodUseCaseResponse(-1, "Order first");
            }
            OrderEntity orderEntity= order.get();

            if(!orderEntity.getUser().getId().equals(userEntity.getId())){
                return new CheckoutCodUseCaseResponse(-1, "Unauthorized to access this order");
            }
            ...
            return new CheckoutCodUseCaseResponse(0,"Order placed on Cash on Delivery");

        }catch(Exception e){
            e.printStackTrace();
            return new CheckoutCodUseCaseResponse(-1,"Internal error");
        }
    }
}
